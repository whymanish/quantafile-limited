...
const ProjectItem = ({ imageSrc, title, description, categories, websiteLink }) => {
  return (
    <div className="mb-12 inline-block border rounded-3xl border-solid border-[#cdcdcd] md:mb-8 lg:mb-10">
      <img src={imageSrc} alt="" className="inline-block rounded-t-3xl" />
      <div className="px-5 py-8 sm:px-6">
        <h5 className="mb-3 text-xl font-bold">{title}</h5>
        <p className="flex-col text-[#808080]">{description}</p>
        <div className="mb-5 mt-6 flex flex-wrap gap-2 md:mb-6 lg:mb-8">
          {categories.map((category, index) => (
            <div
              key={index}
              className="rounded-xl border border-gray-200  p-2 text-xs font-semibold  text-[#636262]"
            >
              <p>{category}</p>
            </div>
          ))}
